...
fn main() {
    ...
    for symbol in symbols {
        let mut close_numbers = numbers.iter().filter(|number| {
        ...
        if close_numbers.clone().count() == 2 {
            let first = close_numbers.next().unwrap();
            let second = close_numbers.next().unwrap();
            valid_total += first.value * second.value;
        }
    }
